...
Here, the most likely model, which has posterior probability of 0.0068, includes an intercept and the variables *runtime*, *thtr_rel_year*, *imdb_rating* and *critics_score*.

We amy also visualize the posterior distribution of the coefficients under the model averaging approach for the most likely model.

```{r cache = TRUE}
par(mfrow = c(2, 2))
coef_audience <- coefficients(bma_audience)
plot(coef_audience, subset = c(4, 36, 51, 53), ask = FALSE)
```

Below, you'll find the 95% credible intervals for these coefficients.

```{r}
confint(coef_audience)[c(4, 36, 51, 53), ]
```

### Interpretation of Model Coefficients

```{r}
print(data.frame("Name" = coef_audience$namesx, "Post Mean" = coef_audience$postmean, "Post SD" = coef_audience$postsd)[c(1, 4, 36, 51, 53), ], row.names = FALSE)
```
...
```{r cache = TRUE}
bma_audience2 <- bas.lm(audience_score ~ ., data = na.omit(movies_ext_reg), prior = "ZS-null", modelprior = uniform(), method = "MCMC")
plot(bma_audience2)
```
